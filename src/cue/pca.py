...
class SpectrumPCA_line():
    ...
    def train_pca(self):
        ...
        for i in range(self.n_batches):
            ...
            if self.parameter_selection is None:
                ...
                log_spectra[log_spectra == 0] = 1e-10
                log_spectra = np.log10(log_spectra[:, self.wav_selection])
                ...
                # load spectra and shift+scale
                normalized_log_spectra = (np.load(self.spectrum_filenames[i])[selection,:] - self.log_spectrum_shift)/self.log_spectrum_scale
    ...
    def transform_and_stack_training_data(self, filename, retain = False):
        ...
        for i in range(self.n_batches):
            ...
            training_parameters.append(np.vstack([np.loadtxt(self.parameter_filenames[j])[:,self.parameter_index] for j in np.arange(self.n_files)+self.n_files*i]))
    ...
    def validate_pca_basis(self, spectrum_filename, parameter_filename=None):
        ...
        if self.parameter_selection is None:
            ...
            log_spectra = np.vstack([np.load(i) for i in spectrum_filename])
            log_spectra[log_spectra == 0] = 1e-10
            log_spectra = np.log10(log_spectra)[selection, self.wav_selection]
            normalized_log_spectra = (log_spectra - self.log_spectrum_shift)/self.log_spectrum_scale
...
class SpectrumPCA_cont():
    """
    SPECULATOR PCA compression class
    """
    
    def __init__(self, n_wavelengths, n_pcas, n_batches, spectrum_filenames, 
                 parameter_filenames, parameter_index=[2,3,4,5,6,7,8,11,12,14,15,16], parameter_selection = None):
        """
        Constructor.
        :param n_wavelengths: number of wavelengths in the modelled SEDs
        :param n_pcas: number of PCA components
